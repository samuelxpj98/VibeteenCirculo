...
export const MissionHQ: React.FC<MissionHQProps> = ({ actions, onClose }) => {
  ...
  const pulseSpeed = Math.max(1, 4 - (stats.total / 50)); // Pulsa mais rápido conforme mais ações
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-[#050505] text-white overflow-hidden font-sans flex flex-col">
      ...
      <div className="absolute inset-0 opacity-10 pointer-events-none">
        ...
        <div className="absolute top-0 left-0 w-full h-full bg-gradient-to-b from-primary/20 via-transparent to-primary/10"></div>
      </div>

      {/* Header Estilo Sci-Fi */}
      <header className="relative z-10 p-8 flex items-center justify-between border-b border-white/10 bg-black/40 backdrop-blur-md">
        ...
        <div className="flex items-center gap-8">
           <div className="text-right hidden md:block">
             <p className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Localização</p>
             <p className="text-sm font-bold uppercase italic">QG Vibe Teen - Weekly Meeting</p>
           </div>
      ...
      {/* Grid Principal */}
      <main className="flex-1 relative z-10 grid grid-cols-12 gap-6 p-8">
        
        {/* Coluna Esquerda: Stats Atômicos */}
        ...
        <div className="col-span-12 lg:col-span-6 flex flex-col items-center justify-center relative">
          ...
          <div className="mt-16 w-full max-w-md">
            ...
            <p className="text-[9px] font-black uppercase tracking-[0.3em] text-center mt-4 text-zinc-600 italic">Meta Semanal: 100 Impactos</p>
          </div>
        </div>

        {/* Coluna Direita: Log de Atividade e Meta */}
        <div className="col-span-12 lg:col-span-3 flex flex-col gap-6">
              ...
              <div className="flex-1 overflow-hidden relative">
                <div className="absolute inset-0 flex flex-col gap-4">
                  {actions.slice(0, 6).map((a, i) => (
                    <div key={i} className="flex items-center gap-4 p-4 bg-white/5 rounded-2xl border border-white/5 animate-in slide-in-from-right duration-500" style={{ animationDelay: `${i * 100}ms` }}>
                      ...
                      <div>
                        <p className="text-xs font-black uppercase italic leading-none">{ACTION_CONFIG[a.action].label} Detectado</p>
                        <p className="text-[9px] font-bold text-zinc-500 uppercase mt-1">Impacto registrado no Reino</p>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <footer className="relative z-10 h-24 border-t border-white/10 bg-black/80 flex items-center overflow-hidden">
        ...
        <div className="flex-1 whitespace-nowrap animate-marquee">
          ...
          <span className="text-2xl font-black italic uppercase tracking-tighter text-zinc-400 px-8 opacity-40">
            • Ação Simples, Impacto Eterno • 
          </span>
          <span className="text-2xl font-black italic uppercase tracking-tighter text-zinc-400 px-8">
